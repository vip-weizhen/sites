const config = {
  ...
  search_engine: [
    ...
    { name: "音乐",    template: "https://www.gequhai.com/s/$s" },
  ...
  lists: [
    ...
    },
    {
      ...
      list: [
        ...
        { url: "https://url.weizhen.xyz/node", name: "FQ代理节点", desc: "科学上网节点订阅链接",    icon: "https://obs.weizhen.xyz/favicon/v2rayn.png" },
...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box}
    ...
    .kt-float-card{cursor:default!important;transform-origin:center center;will-change:left,top,transform}
    .ui.cards>.card.kt-flash{box-shadow:0 0 0 3px rgba(255,255,255,.9),0 0 34px 10px rgba(var(--kt-flash-color,120,180,255),.9)!important}
    body.dark-mode .ui.cards>.card.kt-flash{box-shadow:0 0 0 3px rgba(255,255,255,.7),0 0 34px 10px rgba(var(--kt-flash-color,99,179,237),.9)!important}
...
<body style="margin:0;padding:0;">
  ...
  <script>
    ...
    /* ===== 彩蛋：桌面端30秒无操作，卡片自由漂浮+碰撞+闪烁 ===== */
    (function(){
      if(!window.matchMedia('(hover:hover) and (pointer:fine)').matches) return;

      var cards = [], state = [], floating = false, rafId = null, idleTimer = null, mainEl = null;
      var IDLE_MS = 30000;
      var FLASH_COLORS = ['120,180,255','255,150,205','160,255,175','255,205,110','190,140,255','120,235,235','255,140,140'];

      function initCards(){
        cards = Array.prototype.slice.call(document.querySelectorAll('.ui.four.stackable.cards>.card'));
      }

      function flash(node){
        var now = performance.now();
        if(node._ktFlashUntil && now < node._ktFlashUntil) return;
        var c = FLASH_COLORS[Math.floor(Math.random() * FLASH_COLORS.length)];
        node.style.setProperty('--kt-flash-color', c);
        node.classList.add('kt-flash');
        node._ktFlashUntil = now + 380;
        setTimeout(function(){ node.classList.remove('kt-flash'); node._ktFlashUntil = 0; }, 380);
      }

      function startFloating(){
        if(floating || !cards.length) return;
        var vw0 = window.innerWidth, vh0 = window.innerHeight;
        // 只挑出触发那一刻真正在视口内可见的卡片，避免把滚动条外看不见的卡片瞬间拽出来聚成一团
        var visible = cards.filter(function(node){
          var r = node.getBoundingClientRect();
          return r.bottom > 0 && r.top < vh0 && r.right > 0 && r.left < vw0;
        });
        if(!visible.length) return;
        floating = true;
        mainEl = document.querySelector('main');
        if(mainEl) mainEl.style.minHeight = mainEl.offsetHeight + 'px';

        state = visible.map(function(node){
          var r = node.getBoundingClientRect();
          node.setAttribute('data-orig-style', node.getAttribute('style') || '');
          node.classList.add('kt-float-card');
          node.style.setProperty('position', 'fixed', 'important');
          node.style.setProperty('left', r.left + 'px', 'important');
          node.style.setProperty('top', r.top + 'px', 'important');
          node.style.setProperty('width', r.width + 'px', 'important');
          node.style.setProperty('height', r.height + 'px', 'important');
          node.style.setProperty('margin', '0', 'important');
          node.style.transition = 'none';
          node.style.zIndex = '500';
          node.style.pointerEvents = 'none';
          var angle = Math.random() * Math.PI * 2;
          var speed = 0.55 + Math.random() * 1.0;
          var rotSpeed = (Math.random() < 0.5 ? -1 : 1) * (0.15 + Math.random() * 0.35);
          return {
            el: node,
            x: r.left, y: r.top, w: r.width, h: r.height,
            origX: r.left, origY: r.top,
            vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
            rot: 0, rotSpeed: rotSpeed
          };
        });
      ...
      function tick(){
        ...
        // 卡片间碰撞：按重叠深度把两张卡片正确分开，避免卡在一起反复抖动
        ...
        state.forEach(function(s){
          ...
          if(s.x + s.w >= vw){ s.x = vw - s.w; s.vx = -Math.abs(s.vx); bounced = true; }
          if(s.y <= 0){ s.y = 0; s.vy = Math.abs(s.vy); bounced = true; }
          if(s.y + s.h >= vh){ s.y = vh - s.h; s.vy = -Math.abs(s.vy); bounced = true; }
          s.el.style.setProperty('left', s.x + 'px', 'important');
          s.el.style.setProperty('top', s.y + 'px', 'important');
          s.el.style.transform = 'rotate(' + s.rot.toFixed(2) + 'deg)';
          if(bounced) flash(s.el);
        });
        rafId = requestAnimationFrame(tick);
      }
      ...
      function stopFloating(){
        ...
        var toClean = state;
        state = [];
        var localMain = mainEl;
        mainEl = null;
        var pending = toClean.length;

        function maybeClearMain(){
          pending--;
          if(pending <= 0 && localMain){ localMain.style.minHeight = ''; }
        }
        if(!toClean.length){ if(localMain) localMain.style.minHeight = ''; return; }

        toClean.forEach(function(s){
          var node = s.el;
          node.style.transition = 'left .45s cubic-bezier(.22,1,.36,1), top .45s cubic-bezier(.22,1,.36,1), transform .45s cubic-bezier(.22,1,.36,1)';
          void node.offsetWidth; // 强制回流，确保过渡生效
          node.style.setProperty('left', s.origX + 'px', 'important');
          node.style.setProperty('top', s.origY + 'px', 'important');
          ...
          function finish(){
            ...
            node.classList.remove('kt-flash', 'kt-float-card');
            var orig = node.getAttribute('data-orig-style');
            if(orig){ node.setAttribute('style', orig); } else { node.removeAttribute('style'); }
            node.removeAttribute('data-orig-style');
            maybeClearMain();
          }
      ...
      function resetIdle(){
        if(floating) stopFloating();
        clearTimeout(idleTimer);
        idleTimer = setTimeout(startFloating, IDLE_MS);
      }

      document.addEventListener('DOMContentLoaded', function(){
        initCards();
        resetIdle();
        ['mousemove','mousedown','wheel','keydown','scroll'].forEach(function(evt){
          document.addEventListener(evt, resetIdle, { passive: true });
        });
        window.addEventListener('resize', function(){ if(floating) stopFloating(); resetIdle(); });
      });
    })();
  </script>
